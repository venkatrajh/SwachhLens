      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1 mb-1">
          <span className="text-[11px] font-mono font-bold text-[#168A5B] dark:text-[#39B77A]">
            {report.display_id || formatReportId(report.id, report.created_at || report.reported_at || report.timestamp)}
          </span>
        ...
        <div className="flex items-center gap-1 text-[11px] text-[#64736A] dark:text-[#A9BBB1] truncate mt-1.5">
          <MapPin className="w-3 h-3 flex-shrink-0 text-[#168A5B] dark:text-[#39B77A]" />
          <span className="truncate">
            {report.address_label || `${report.latitude.toFixed(4)}°, ${report.longitude.toFixed(4)}°`}
          </span>
        </div>

        {report.description && (
          <p className="text-[11px] text-[#64736A] dark:text-[#A9BBB1] truncate mt-1">
            {report.description}
          </p>
        )}
      </div>
