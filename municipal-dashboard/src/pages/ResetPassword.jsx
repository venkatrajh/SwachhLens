...
import { Shield, ArrowRight, ArrowLeft, Lock, AlertCircle, CheckCircle2, Sun, Moon } from 'lucide-react';
...
export const ResetPassword = () => {
  ...
  const handleReset = async (e) => {
    ...
    if (!token) {
      setErrorMessage('Reset token is missing. Please use the complete link from your email.');
      return;
    }
    ...
    try {
      ...
      setErrorMessage(err.message || 'Password reset failed. The token may be expired or already used.');
    } finally {
        ...
        >
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '4px' }}>
              Create a secure password for your municipal account
            </p>
          </div>
          ...
          {(!token || errorMessage) && (
            ...
            >
              <AlertCircle size={14} />
              <span>{!token ? 'Missing reset token in link.' : errorMessage}</span>
            </div>
          )}
          ...
          {isSuccess ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              ...
              >
                ...
                <span>
                  Password reset successfully! You can now log in to the Municipal Command Center.
                </span>
              </div>
