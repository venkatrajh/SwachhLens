...
export const FloatingNavbar = () => {
  ...
  const handleNotificationClick = async (notification) => {
    ...
    if (notification.report_id) {
      navigate(`/complaints/${notification.report_id}`);
      setShowNotifications(false);
    }
  };
  ...
  const displayName = user?.name || 'Authorized Official';
                  ...
                  {notificationsLoading ? (
                    ...
                    notifications.map(n => (
                      <div
                        ...
                        style={{
                          padding: '10px',
                          backgroundColor: n.is_read ? 'var(--surface-secondary)' : 'var(--surface-active)',
                          borderRadius: 'var(--radius-sm)',
                          border: n.is_read ? '1px solid var(--glass-border-subtle)' : '1px solid var(--accent-border)',
                          fontSize: '12px',
                          cursor: 'pointer',
                          transition: 'background-color var(--transition-fast)'
                        }}
                        onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'var(--surface-hover)'; }}
                        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = n.is_read ? 'var(--surface-secondary)' : 'var(--surface-active)'; }}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontWeight: 600, color: n.is_read ? 'var(--text-secondary)' : 'var(--text-primary)' }}>
                            {n.title}
                          </span>
                        ...
                        <p style={{ color: n.is_read ? 'var(--text-muted)' : 'var(--text-secondary)', marginTop: '3px' }}>
                          {n.message}
                        </p>
                      </div>
                    ))
