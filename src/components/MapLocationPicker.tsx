...
export const MapLocationPicker: React.FC<MapLocationPickerProps> = ({
  ...
  useEffect(() => {
    ...
    marker.on('dragend', () => {
      ...
      setAddressLabel(`Map Selected Location (${pos.lat.toFixed(4)}, ${pos.lng.toFixed(4)})`);
    });

    map.on('click', (e) => {
      marker.setLatLng(e.latlng);
      setCurrentCoords({ lat: e.latlng.lat, lng: e.latlng.lng });
      setAddressLabel(`Map Selected Location (${e.latlng.lat.toFixed(4)}, ${e.latlng.lng.toFixed(4)})`);
    });
  ...
  const handleUseCurrentLocation = async () => {
    ...
    try {
      ...
      if (mapInstanceRef.current && markerRef.current) {
        mapInstanceRef.current.flyTo([mockLoc.latitude, mockLoc.longitude], 17);
        markerRef.current.setLatLng([mockLoc.latitude, mockLoc.longitude]);
        setCurrentCoords({ lat: mockLoc.latitude, lng: mockLoc.longitude });
        setAddressLabel('Current Locality (GPS)');
      }
    } finally {
      setIsLocating(false);
    }
